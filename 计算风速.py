...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		...
		help="可选：开始年份（含），如 1990",
	)
	parser.add_argument(
		"--end-year",
		type=int,
		default=DEFAULT_END_YEAR,
		help="可选：结束年份（含），如 2019",
	)
...
def main() -> None:
	...
	if args.start_year is not None and args.end_year is not None and args.start_year > args.end_year:
		raise ValueError(
			f"年份范围非法: start-year({args.start_year}) 不能大于 end-year({args.end_year})"
		)
	...
	if not files:
		...
		if args.start_year is not None and args.end_year is not None:
			year_desc = f"{args.start_year}-{args.end_year}"
		elif args.start_year is not None:
			year_desc = f">={args.start_year}"
	...
	if args.start_year is not None or args.end_year is not None:
		if args.start_year is not None and args.end_year is not None:
			print(f"读取年份范围: {args.start_year}-{args.end_year}")
		elif args.start_year is not None:
			print(f"读取年份范围: >= {args.start_year}")
		else:
			print(f"读取年份范围: <= {args.end_year}")
	if args.station_id is not None:
