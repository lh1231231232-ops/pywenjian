...
def combine_images_with_dates(folder_path, output_folder, square_size=512):
    """
    合并相同编号和字母前缀的PNG文件，并在每个小图右下角添加日期水印
    ...
    # 处理每个编号组的文件
    for (letter_prefix, number), files in file_groups.items():
        print(f"处理编号 {letter_prefix}{number} 的 {len(files)} 个文件")
        ...
        # 保存合并后的图片到指定输出文件夹
        # 使用字母前缀和数字编号作为输出文件名
        output_filename = f"{letter_prefix}{number}#.png"
